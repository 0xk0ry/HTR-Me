...
def visualize_chunking_process():
    ...
    print(f"   Ignored region size: {ignored_size}px = {ignore_patches} patches")
    ...
    for i, (features, (start_px, end_px, left_pad_px, content_end_in_chunk_px)) in enumerate(zip(chunk_features, chunk_positions)):
        ...
        if len(chunk_positions) == 1:
            ...
            if actual_content_width_px < chunker.chunk_width - left_pad_px:
                content_ratio = (left_pad_px + actual_content_width_px) / chunker.chunk_width
                actual_content_patches = max(1, int(content_ratio * total_patches))
            ...
            print(f"   - Right padding removed: {total_patches - end_idx} patches")
            ...
            if i == 0:
                ...
                print(f"   - First chunk: valid patches [{start_idx}:{end_idx}]")
                print(f"   - Left padding removed: {start_idx} patches")
                print(f"   - Right overlap ignored: {total_patches - end_idx} patches")
                ...
                print(f"   - Last chunk: valid patches [{start_idx}:{end_idx}]")
                ...
                print(f"   - Middle chunk: valid patches [{start_idx}:{end_idx}]")
                print(f"   - Left overlap ignored: {start_idx} patches")
                print(f"   - Right overlap ignored: {total_patches - end_idx} patches")
    ...
    merged_features = model._merge_chunk_features(chunk_features, chunk_positions)
    print(f"\n8. Final Merged Features:")
    print(f"   Shape: {merged_features.shape} (Total_time_steps x Feature_dim)")
...
def create_visualization_plot(results):
    ...
    ignored_counts = [total - valid for valid, total in zip(valid_counts, total_counts)]
    
    x = range(len(valid_counts))
    axes[1, 1].bar(x, valid_counts, label='Valid Patches', alpha=0.7)
    axes[1, 1].bar(x, ignored_counts, bottom=valid_counts, label='Ignored Patches', alpha=0.7)
    ...
    if results['merged_features'].shape[0] > 0:
        ...
        im = axes[2, 0].imshow(sample_features_np.T, aspect='auto', cmap='viridis')
    ...
    axes[2, 1].text(0.1, 0.7, f'Final Output Shape:', fontsize=12, weight='bold')
    axes[2, 1].text(0.1, 0.6, f'Logits: {logits_shape}', fontsize=10)
    axes[2, 1].text(0.1, 0.5, f'Format: (T_max, Batch, Vocab)', fontsize=10)
    axes[2, 1].text(0.1, 0.4, f'Sequence Length: {results["sequence_lengths"]}', fontsize=10)
    axes[2, 1].text(0.1, 0.3, f'Ready for CTC Loss', fontsize=10, style='italic')
...
def run_comprehensive_test():
    ...
    # Save plot
    output_path = os.path.join(os.path.dirname(__file__), 'chunk_visualization.png')
    fig.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nVisualization saved to: {output_path}")
    
    # Save detailed results
    results_path = os.path.join(os.path.dirname(__file__), 'chunk_analysis_results.txt')
