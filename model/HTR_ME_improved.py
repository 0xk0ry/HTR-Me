...
class HTRModel(nn.Module):
    ...
    def __init__(self, vocab_size, max_length=256, target_height=40, chunk_width=320,
        ...
        # Cross-chunk context fusion
        self.context_size = 10  # Number of context tokens to share between chunks
        ...
        # Cross-chunk attention for better boundary handling
        self.cross_chunk_attention = nn.MultiheadAttention(
    ...
    def _apply_cross_chunk_context(self, chunk_features):
        """Apply cross-chunk context to enhance boundary handling
        
        Each chunk receives context from its neighbors through:
        1. Simple context concatenation and fusion
        2. Cross-attention mechanism for adaptive context integration
        """
        ...
        for i, current_features in enumerate(chunk_features):
            enhanced_chunk = current_features
            
            # Method 1: Simple context fusion with neighboring chunks
            if i > 0:  # Has previous chunk
                prev_context = chunk_features[i-1][-self.context_size:].clone()
                if prev_context.size(0) > 0:
                    enhanced_chunk = self._fuse_with_context(enhanced_chunk, prev_context, 'prev')
            
            if i < len(chunk_features) - 1:  # Has next chunk  
                next_context = chunk_features[i+1][:self.context_size].clone()
                if next_context.size(0) > 0:
                    enhanced_chunk = self._fuse_with_context(enhanced_chunk, next_context, 'next')
            
            # Method 2: Cross-attention with neighboring chunks (more sophisticated)
            if len(chunk_features) > 1 and self.training:  # Only during training for efficiency
                enhanced_chunk = self._apply_cross_attention(enhanced_chunk, chunk_features, i)
            
            enhanced_features.append(enhanced_chunk)
        
        return enhanced_features
    
    def _fuse_with_context(self, chunk_features, context_features, context_type):
        """Fuse chunk features with context using learned fusion"""
        chunk_len = chunk_features.size(0)
        context_len = context_features.size(0)
        
        if context_len == 0:
            return chunk_features
        
        # Determine fusion region based on context type
        if context_type == 'prev':
            # Fuse context with beginning of current chunk
            fusion_len = min(self.context_size, chunk_len)
            target_region = chunk_features[:fusion_len]  # [fusion_len, feature_dim]
            
            # Repeat context if needed to match target region
            if context_len < fusion_len:
                context_features = context_features.repeat(
                    (fusion_len + context_len - 1) // context_len, 1)[:fusion_len]
            else:
                context_features = context_features[:fusion_len]
                
        else:  # context_type == 'next'
            # Fuse context with end of current chunk
            fusion_len = min(self.context_size, chunk_len)
            target_region = chunk_features[-fusion_len:]  # [fusion_len, feature_dim]
        ...
        fused_input = torch.cat([target_region, context_features], dim=-1)  # [fusion_len, 2*feature_dim]
        fused_output = self.context_fusion(fused_input)  # [fusion_len, feature_dim]
        
        # Replace the target region with fused features
        enhanced_chunk = chunk_features.clone()
        if context_type == 'prev':
            enhanced_chunk[:fusion_len] = fused_output
        else:
            enhanced_chunk[-fusion_len:] = fused_output
            
        return enhanced_chunk
    
    def _apply_cross_attention(self, current_features, all_chunk_features, current_idx):
        """Apply cross-attention between current chunk and its neighbors"""
        # Collect context from neighboring chunks
        context_chunks = []
        
        # Add previous chunk context
        if current_idx > 0:
            prev_context = all_chunk_features[current_idx - 1][-self.context_size:]
            if prev_context.size(0) > 0:
                context_chunks.append(prev_context)
        
        # Add next chunk context  
        if current_idx < len(all_chunk_features) - 1:
            next_context = all_chunk_features[current_idx + 1][:self.context_size]
            if next_context.size(0) > 0:
                context_chunks.append(next_context)
        
        if not context_chunks:
            return current_features
            
        # Concatenate all context
        context = torch.cat(context_chunks, dim=0)  # [total_context_len, feature_dim]
        
        # Apply cross-attention
        # current_features as query, context as key and value
        query = current_features.unsqueeze(0)  # [1, seq_len, feature_dim]
        key_value = context.unsqueeze(0)  # [1, context_len, feature_dim]
        
        attended_output, _ = self.cross_chunk_attention(
            query, key_value, key_value
        )  # [1, seq_len, feature_dim]
        
        attended_features = attended_output.squeeze(0)  # [seq_len, feature_dim]
        
        # Residual connection with original features
        enhanced_features = current_features + 0.1 * attended_features  # Small residual weight
        
        return enhanced_features

    def forward(self, images, targets=None, target_lengths=None):
        """Forward pass through the model with cross-chunk context

        ENHANCED with cross-chunk context:
        - Each chunk can peek at context from adjacent chunks
        - Cross-attention mechanism for better boundary handling
        - Maintains all previous fixes for CTC and tensor handling
        """
        ...
        for i in range(batch_size):
            ...
            # Apply cross-chunk context enhancement
            enhanced_chunk_features = self._apply_cross_chunk_context(chunk_features)

            # Merge chunks by removing padding and ignored regions
            merged_features = self._merge_chunk_features(
                enhanced_chunk_features, chunk_positions)  # [T_total, C]
...
def create_model_example():
    """Example of how to create and initialize the model with cross-chunk context"""
    vocab_size = len(DEFAULT_VOCAB)

    # Create model with enhanced cross-chunk context
    model = HTRModel(
